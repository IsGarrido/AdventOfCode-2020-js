...
data = `LLLLLLLLL.L.LLLL.LLLL.LLLLLLLL.LLLLLLLLLLLLL.LLLL.LLLLLL.LLLL..LLLLLL.LLLLLLLL.LLLLL.LL.LLLLLLLLLL
...
data
	.split("\n")
	.forEach(line => lastMap.push(line.split("")));
...
function getAdjacentOccupied(grid, y, x) {
	...
	DIRECTIONS.forEach(dir => {
		...
		while (posX > -1 && posY > -1 && posX < grid[0].length && posY < grid.length) {
			let val = get(grid, posY, posX);

			if (val === STATUS.FREE || val === STATUS.USED) {
				c += (val === STATUS.USED) ? 1 : 0;
				break;
			}
...
function run(m) {
	...
	for (let y = 0; y < m.length; y++) {
		for (let x = 0; x < m[y].length; x++) {
			let adjacents = getAdjacentOccupied(m, y, x);
			changed += set(y, x, adjacents);
		}
	}
