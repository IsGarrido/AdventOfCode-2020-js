...
function RunWire(w, me){
	...
	for( let i = 0; i < w.length; i++){
		...
		let op = w[i];
		let dir = op[0];
		let qty = op.substr(1,op.length);

		//console.log(dir);
		if(dir == "R"){
			for(let step = 0; step < qty; step++){
				currX++;
				Visit(currX, currY,me);
			}
		} else if(dir == "L"){
			for(let step = 0; step < qty; step++){
				currX--;
...
let distances = common.map( pos => Manhattan(pos[0], pos[1]) );
